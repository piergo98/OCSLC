...
class SwiLin:
    ...
    def __init__(self, np, nx, nu, auto=False) -> None:
        ...
        if auto:
            self.Nu = 1
            self.u = ca.SX.zeros(self.Nu, self.n)
            
        self.delta = ca.SX.sym('delta', self.n)
    ...
    def compute_integral(self, A, B, tmin, tmax):
        ...
        # print(ca.symvar(tmax))
        # print(ca.symvar(tmax).is_symbolic())
        # print(f"s: {s} ")
        # print(type(s))
        # print(f"f: {f} ")
        # print(type(f))
    ...
    def mat_exp_prop(self, index):
        ...
        Hk = ca.SX.sym('Hi', self.Nx + 1, self.Nx + 1)
        Hk = 0*Hk
    ...
    def transition_matrix(self, phi_a, phi_f):
        ...
        phi = ca.SX.sym('phi', self.Nx+1, self.Nx+1)
        phi = 0*phi
    ...
    def D_matrix(self, index, Q, tau_i):
        ...
        tau_i (ca.SX): The sum of the phases elapsed.
        ...
        Hij_t = ca.SX.sym('Hij_t', self.Nx + 1, self.Nx + 1)
        Hij_t = 0*Hij_t
        for k in range(ui.shape[0]):
    ...
    def C_matrix(self, index, Q):
        ...
        # Define the M matrix
        M = ca.SX.sym('M', self.Nx + 1, self.Nx + 1)
        M = 0*M
    ...
    def cost_function(self, R, x0):
        ...
        J = self.G_matrix(R) + np.transpose(x0) @ self.S[0] @ x0
    ...
    def grad_cost_function(self, index, R):
        ...
        x_aug = ca.SX.sym('x_aug', self.Nx + 1)
        x_next_aug = ca.SX.sym('x_next_aug', self.Nx + 1)
