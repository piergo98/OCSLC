...
from ocslc.receding_horizon_mpc import RecedingHorizonMPC
from ocslc.utils.mpc_animator import MPCAnimator
...
def closed_loop_tracking_mpc():
    ...
    # Cost weights
    Q = np.diag([100.0, 1.0])      # Position more important than velocity
    ...
    def reference_trajectory(t):
        ...
        #                 1.0 * np.cos(0.5 * t)])
        return np.array([1.5, 5.0])  # Fixed point reference
    ...
    for step in range(n_sim_steps):
        ...
        A_d = mpc_controller.mpc.autonomous_evol[0](dt_control).full()
        print(A_d, type(A_d))
        # A_d = expM[:n_states, :n_states]
        B_d = mpc_controller.mpc.forced_evol[0](u_applied, dt_control).full().flatten()
        print(B_d, type(B_d))
        # B_d = expM[:n_states, n_states:]
        
        # System evolution using discretized matrices     
        x_next = A_d @ x_current + B_d + w
        print(type(x_next))
